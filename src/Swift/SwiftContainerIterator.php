 ...
 * @author Joan Fabrégat <joan@codeinc.fr>
 */
class SwiftContainerIterator implements \Iterator {
	...
	private function listObjects(int $retryOnFailure = self::RETRY_ON_FAILURE) {
		try {
			...
			if ($retryOnFailure > 0) {
				...
				throw new SwiftContainerIteratorException($this->swiftContainer,
					"Error while listing the objects of the Swift container \"{$this->swiftContainer->getContainerName()}\"",
					$exception);
			}
		}
	}
